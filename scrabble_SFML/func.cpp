...
void mainWindow () {
    ...
    tscore1 = Text("Score: 0", font, 60);
    tscore2 = Text("Score: 0", font, 60);
    Text textMenu[4];
    Sprite board;
    Texture boardTex;
...
//                            onBoard = true;
        ...
        window.clear(Color(147, 112, 219));
...
void menuFunc (Text boardText[BOARD_SIZE][BOARD_SIZE], bool& gorizontal, int& lastI, int& lastJ, letters* lett_play1, letters* lett_play2, Text* lett1, Text* lett2, RectangleShape recBoard[BOARD_SIZE][BOARD_SIZE], RectangleShape* rectangles1, RectangleShape* rectangles2) {
    switch (selectedMenuIndex) {
        ...
        case 2:
            ...
            //End action
            //
//            if (Currentword == "") {// Stap pass
//                if (secondPlayer) {
//                    secondPlayer = false;//                TODO
//                } else {
//                    secondPlayer = true;
//                }
//                selectedItemIndex = 0;
...
//            }
            ...
            bool touchEachOther = false;
            if (Currentword != firstLetters || firstRound) {
                touchEachOther = true;
                firstRound = false;
            }
            if (!correct) {
            ...
            if (gorizontal) {
                j = lastJ;
                for (int i = 0; i < BOARD_SIZE; i++) {
                    if (ind[i] && (boardText[lastI + 1][i].getString() != " " || boardText[lastI - 1][i].getString() != " ")) {
                        touchEachOther = true;
                        while (boardText[j - 1][i].getString() != " " && j >= 0) {
                            j--;
                        }
                        while (boardText[j][i].getString() != " " && j < BOARD_SIZE) {
                            Currentword += boardText[j][i].getString();
                            j++;
                        }
                        if (Currentword != "") {
                            correct = search_words(Currentword);
                        }
                        Currentword = "";
                        if (!correct) {
                            i = BOARD_SIZE;
                ...
                j = lastI;
                for (int i = 0; i < BOARD_SIZE; i++) {
                    if (ind[i] && (boardText[i][lastJ + 1].getString() != " " || boardText[i][lastJ - 1].getString() != " ")) {
                        touchEachOther = true;
                        while (boardText[i][j - 1].getString() != " " && j >= 0) {
                            j--;
                        }
            ...
            //CHECKING END
            if(correct && touchEachOther){// Correct word
//                lastJ = 0;
//                lastI = 0;
                if (!secondPlayer) {
                    copy_player_letters(copyLetters, lett_play2);
                    rectangles1[selectedItemIndex].setFillColor(Color(50, 205, 50));
                    x = 515;
                    y = 35;
                    for (int i = 0; i < 7; i++) {
                    ...
                    rectangles2[selectedItemIndex].setFillColor(Color(50, 205, 50));
                    secondPlayer = false;
                    x = 515;
                    for (int i = 0; i < 7; i++) {
                ...
                if (secondPlayer) {
                    for (int i = 0; i < firstLetters.length(); i++) {
                        tmpScore += scoreForLetter(firstLetters[i]);
                    }
                    if (gorizontal) {
                        for (int i = 0; i < BOARD_SIZE; i++) {
                            if (ind[i]) {
                                switch (scoreArray[lastI][i]) {
                                    case 1:
                                        tmpScore *= 2;
                                        break;
                                    case 2:
                    ...
                    score2 += tmpScore;
                    tmpScoreStr += to_string(score2);
                    tscore1.setString(tmpScoreStr);
                } else {
                    for (int i = 0; i < firstLetters.length(); i++) {
                        tmpScore += scoreForLetter(firstLetters[i]);
                    }
                    ...
                    score1 += tmpScore;
                    tmpScoreStr += to_string(score1);
                    tscore2.setString(tmpScoreStr);
                }
                selectedItemIndex = 0;
            } else {
                // Incorrect word
...
void add_letter(letters* letter) {
    ...
    char alphabet[98] = { 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'b', 'b', 'c', 'c', 'd', 'd', 'd', 'd', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'f', 'f', 'g', 'g', 'g', 'h', 'h', 'i', 'i', 'i', 'i', 'i', 'i', 'i', 'i', 'i', 'j', 'k', 'l', 'l', 'l', 'l', 'm', 'm', 'n', 'n', 'n', 'n', 'n', 'n', 'o', 'o', 'o', 'o', 'o', 'o', 'o', 'o', 'p', 'p', 'q', 'r', 'r', 'r', 'r', 'r', 'r', 's', 's', 's', 's', 't', 't', 't', 't', 't', 't',   'u', 'u', 'u', 'u', 'v', 'v', 'w', 'w', 'x', 'y', 'y', 'z' };
    letter->letter = alphabet[rand() % 98];
...
void textInit (Text*& lett1, Text*& lett2, letters*& lett_play1, letters*& lett_play2, Text boardText[BOARD_SIZE][BOARD_SIZE], Font& font, Text& player1, Text& player2, Text& tscore1, Text& tscore2, Text textMenu[4]) {
    ...
    textMenu[0].setPosition(1430, 300);
    textMenu[1].setPosition(1430, 550);
    textMenu[2].setPosition(1433, 765);
    textMenu[3].setPosition(1425, 1050);
    textMenu[0].setString("        Add");
    textMenu[1].setString("        Help");
    textMenu[2].setString("        End\n      action");
...
void helper () {
    RenderWindow window(VideoMode(1500,500), "SCRABBLE");
    Font font;
    font.loadFromFile("righteous.ttf");
    Text game;
    game = Text("COMING SOON", font, 200);
    game.setPosition(50, 100);
    game.setStyle(Text::Bold);
    game.setOutlineThickness(4);
    game.setOutlineColor(Color::White);
    game.setFillColor(Color(0, 100, 0));
