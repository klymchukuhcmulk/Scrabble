...
void menuFunc (Text boardText[BOARD_SIZE][BOARD_SIZE], bool& gorizontal, int& lastI, int& lastJ, letters* lett_play1, letters* lett_play2, Text* lett1, Text* lett2, RectangleShape recBoard[BOARD_SIZE][BOARD_SIZE], RectangleShape* rectangles1, RectangleShape* rectangles2) {
    ...
    switch (selectedMenuIndex) {
        ...
        case 3:
            ...
            cout << "WORD first : " << firstLetters << endl;
            Currentword = "";
            int i = lastI;
            int j = lastJ;
            ...
            while (currentJ < Currentword.length() && gorizontal && correct) {
                if (boardText[lastI + 1][j].getString() != " " || boardText[lastI - 1][j].getString() != " ") {
                    i = lastI;
                    j = currentJ;
                    while (boardText[i + 1][j].getString() != " " && j >= 0) {
                        i--;
                    }
                    while (boardText[i][j].getString() != " " && j < BOARD_SIZE) {
                        chackingWord += boardText[i][j].getString();
                        i++;
                    }
                    correct = search_words(chackingWord);
                    cout << chackingWord << " :CHECKING WORD" << endl;
                }
                chackingWord = "";
                currentJ++;
            }
            while (currentI < Currentword.length() && !gorizontal && correct) {
                if (boardText[lastI + 1][j].getString() != " " || boardText[lastI - 1][j].getString() != " ") {
                    i = currentI;
                    j = lastJ;
                    while (boardText[i - 1][j].getString() != " " && i >= 0) {
...
//                Currentword = firstLetters;
                ...
                if (Currentword.length() != firstLetters.length()) {
                    if (gorizontal) {
                        int j = lastJ;
                        while (boardText[lastI][j].getString() != " ") {
                            if (boardText[lastI - 1][j].getString() == " " && boardText[lastI + 1][j].getString() == " ") {
                                boardText[lastI][j].setString(" ");
                                recBoard[lastI][j].setFillColor(Color::Transparent);
                            }
                            j--;
                        }
                    } else {
                    ...
                    reverse(firstLetters.begin(), firstLetters.end());
                    if (gorizontal) {
                        int j = lastJ;
                        int i = 0;
                        while (boardText[lastI][j].getString() == firstLetters[i] && i < firstLetters.length()) {
                            boardText[lastI][j].setString(" ");
                            recBoard[lastI][j].setFillColor(Color::Transparent);
                            j--;
                            i++;
                        ...
                        int i = lastI;
                        int j = 0;
                        while (boardText[i][lastJ].getString() == firstLetters[j] && j < firstLetters.length()) {
                            boardText[i][lastJ].setString(" ");
                            recBoard[i][lastJ].setFillColor(Color::Transparent);
                            i--;
                            j++;
